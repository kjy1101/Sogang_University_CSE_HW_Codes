...
__global__ void Sum_n_elements_Kernel(IN int* d_ArrayElements, OUT int* d_SumOfArrayElements, int N, int Nf) {
    ...
    d_SumOfArrayElements[i] = 0;
    for (j = i - Nf; j <= i + Nf; j++) {
        if (j < 0 || j >= N) {
            temp = 0;
        }
        else {
            temp = d_ArrayElements[j];
        }
        d_SumOfArrayElements[i] += temp;
    }
}
    ...
    for (i = 0; i < N; i++) {
        ...
        for (j = i - Nf; j <= i + Nf; j++) {
            if (j < 0 || j >= N) {
                ...
                temp = p_ArrayElements[j];
            }
            p_SumOfElements_CPU[i] += temp;
        }
    }
}
...
int main()
{
    ...
    FILE* ofp = fopen("Cuda_HW3_output.bin", "wb");
	if (!ofp) {
		fprintf(stderr, "Error: cannot open the output file...\n");
		exit(-1);
	}
    fwrite(&N, sizeof(int), 1, ofp);
    fwrite(&Nf, sizeof(int), 1, ofp);

    for (int i = 0; i < N; i++) {
        fwrite(&h_SumOfArrayElements_GPU[i], sizeof(int), 1, ofp);
    }
...
cudaError_t Sum_n_elements_GPU(IN int* p_ArrayElements, OUT int* p_SumOfElements_GPU, int Nf) {
    ...
    size = N * sizeof(int);
    CUDA_CALL(cudaMalloc(&arr_elements, size))
    CUDA_CALL(cudaMemcpy(arr_elements, p_ArrayElements, size, cudaMemcpyHostToDevice))

    CUDA_CALL(cudaMalloc(&sum_elements, size))
    ...
    Sum_n_elements_Kernel << < dimGrid, dimBlock >> > (arr_elements, sum_elements, N, Nf);
    CHECK_TIME_END_GPU(device_time);
    CHECK_TIME_DEST_GPU();

    CUDA_CALL(cudaDeviceSynchronize())
        CUDA_CALL(cudaMemcpy(p_SumOfElements_GPU, sum_elements, size, cudaMemcpyDeviceToHost))

    cudaFree(arr_elements);
    cudaFree(sum_elements);
