...
__global__ void Mat_Vec_Multiply_Kernel(float* d_VecY, float* d_VecX, float* d_MatA, int Vec_Size) {
	int vec_idx, i, j;
	vec_idx = blockDim.x * blockIdx.x + threadIdx.x;
	for (i = 0; i < Vec_Size; i++) {
		float sum = 0;
		for (j = 0; j < Vec_Size; j++) {
			sum += d_MatA[i * Vec_Size + j] * d_VecX[vec_idx * Vec_Size + j];
		}
		d_VecY[vec_idx * Vec_Size + i] = sum;
	}
}
...
void Mat_Vec_Multiply_GPU(float* p_VecX, float* p_MatA, float* p_VecY_G) {
	...
	*/
	...
	CUDA_CALL(cudaMalloc(&vecX, size))
	CUDA_CALL(cudaMemcpy(vecX, p_VecX, size, cudaMemcpyHostToDevice))

	size = ELEM_PER_VECTOR * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&matA, size))
	CUDA_CALL(cudaMemcpy(matA, p_MatA, size, cudaMemcpyHostToDevice))

	size = n * ELEM_PER_VECTOR * sizeof(float);
	CUDA_CALL(cudaMalloc(&vecY_G, size))
	...
	CHECK_TIME_INIT_GPU();
	CHECK_TIME_START_GPU();
	Mat_Vec_Multiply_Kernel << < dimGrid, dimBlock >> > (vecY_G, vecX, matA, ELEM_PER_VECTOR);
	CHECK_TIME_END_GPU(device_time);
	CHECK_TIME_DEST_GPU();

	CUDA_CALL(cudaDeviceSynchronize())
		CUDA_CALL(cudaMemcpy(p_VecY_G, vecY_G, size, cudaMemcpyDeviceToHost))
