...
int main()
{
	...
	for (int i = 0; i < MAX_POINTS; i++) {
		random_x = (rand() % 201) - 100;
		random_y = (rand() % 201) - 100;
		distance = (random_x * random_x) + (random_y * random_y);
		distance = sqrt(distance);
		if (distance <= 100) {
			Ellipse(CENTER_X + random_x, CENTER_Y + random_y, CENTER_X + random_x + POINTS_RADIUS, CENTER_Y + random_y + POINTS_RADIUS, RGB(255, 0, 0), RGB(255, 0, 0));
			dot_in++;
		}
		else {
			Ellipse(CENTER_X + random_x, CENTER_Y + random_y, CENTER_X + random_x + POINTS_RADIUS, CENTER_Y + random_y + POINTS_RADIUS, RGB(255, 255, 0), RGB(255, 255, 0));
		}
	}
	
	//
	calPi = (dot_in / MAX_POINTS) * 4;
