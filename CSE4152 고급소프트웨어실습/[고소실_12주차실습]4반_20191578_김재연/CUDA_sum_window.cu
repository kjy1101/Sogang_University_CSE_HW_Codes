...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	if (thread_id == 0) {
		for (i = 0; i < Nf; i++) {
			k = id + i - Nf;
			if (k < 0) shared_buffer[i] = 0;
			else shared_buffer[i] = d_ArrayElements[k];
		}
	}
	if (thread_id == BLOCK_SIZE - 1) {
		for (i = BLOCK_SIZE; i < BLOCK_SIZE + Nf; i++) {
			k = id + i - BLOCK_SIZE + 1;
			if (k >= N) shared_buffer[i + Nf] = 0;
			else shared_buffer[i + Nf] = d_ArrayElements[k];
		}
	}
