...
bool ofApp::BFS() {
	//BFS탐색
	//q.push(), q.pop(), q.front(), q.back(), q.size(), q.empty()
	int *visited = new int[HEIGHT*WIDTH];
	for (int i = 0; i < HEIGHT*WIDTH; i++) {
		visited[i] = 0;
	}
	...
	while (!Q.empty()) {
		parent_idx = Q.front();
		cur = vertex[Q.front()]->next;
		Q.pop();
		while (cur != NULL) {
			if (visited[cur->index] == 0) {
				Q.push(cur->index);
				visited[cur->index] = 1;
				parent[cur->index] = parent_idx;
				bfs_other_path[bfs_other_idx] = cur->index;
				bfs_other_path[bfs_other_idx+1] = parent[cur->index];
				bfs_other_idx += 2;
				if (cur->index == target) {
					target_flag = 1;
					break;
			...
			if (target_flag == 1) break;
			cur = cur->next;
		}
	}

	int path_node = HEIGHT * WIDTH - 1;
	bfs_path[0] = path_node;
	bfs_idx = 1;

	while (parent[path_node] != 0) {
		bfs_path[bfs_idx] = parent[path_node];
		bfs_idx++;
		path_node = parent[path_node];
	}
	bfs_path[bfs_idx] = 0;
//	for (int i = 0; i <= bfs_idx; i++) {
//		cout << "bfs path: " << bfs_path[i] << endl;
//	}
...
void ofApp::bfsdraw() {
	...
	for (i = 0; i < bfs_other_idx; i += 2) {
		x1 = (bfs_other_path[i] % WIDTH) * 50 + 25;
		y1 = (bfs_other_path[i] / WIDTH) * 50 + 25;
		x2 = (bfs_other_path[i + 1] % WIDTH) * 50 + 25;
		y2 = (bfs_other_path[i + 1] / WIDTH) * 50 + 25;
		ofDrawLine(x1, y1, x2, y2);
	}

	ofSetColor(200, 150, 0);
	for (i = 0; i < bfs_idx; i++) {
		x1 = (bfs_path[i] % WIDTH) * 50 + 25;
		y1 = (bfs_path[i] / WIDTH) * 50 + 25;
		x2 = (bfs_path[i + 1] % WIDTH) * 50 + 25;
		y2 = (bfs_path[i + 1] / WIDTH) * 50 + 25;
		ofDrawLine(x1, y1, x2, y2);
	}
}