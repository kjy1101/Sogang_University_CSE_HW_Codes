...
void BlockDown(int sig){
	...
	if(auto_flag==1 && CheckToMove(field, nextBlock[0], recRoot->recBlockRotate, recRoot->recBlockY, recRoot->recBlockX)==1){
	...
	if(down_ok==1){
		...
		if(blockY == -1){
			...
			DrawNextBlock(nextBlock);
			DrawField();
			if(auto_flag==1) DrawBlockWithFeatures(blockY, blockX, nextBlock[0], blockRotate);
		}
	}
	alarm(1);
}
...
void recommendedPlay(){
	...
	timer.it_value.tv_usec = 1%1000;
	timer.it_interval.tv_sec = 0;
	timer.it_interval.tv_usec = 1%1000;
...
int modified_recommend(Node* root){
	...
	for(int i=0;i<p_rotate;i++){
		for(int j=0;j<p_position[root->curBlockID][i];j++){
			...
			//fprintf(gfp, "level=%d, i=%d,j=%d,seq=%d,del_line=%d, a_score=%d\n",treenode->level, i,j,seq,del_line,treenode->accumulatedScore);
			if ((del_line!=0)&&(treenode->level<3)) {
			...
			//fprintf(gfp, "curBlockID=%d, score=%d, recBlockX=%d, recBlockY=%d, recBlockRotate=%d\n",recRoot->curBlockID, recRoot->score, recRoot->recBlockX, recRoot->recBlockY, recRoot->recBlockRotate); 
				free(treenode);
				return 9999;
			}
			...
			//시작
			if(treenode->level < VISIBLE_BLOCKS){
				if ( modified_recommend(treenode) == 9999 ) {
					free(treenode);
					return 9999;
				}
			}
