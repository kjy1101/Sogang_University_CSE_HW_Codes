...
process_execute (const char *file_name) 
{
  ...
  real_filename = strtok_r(filename, " ", &save_ptr);

  struct file *file = NULL;
  file = filesys_open (real_filename);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* 명령어 쪼개기(기준: 공백) */
  int argc = 0;
  char file_name_src[256];
  char *token;
  char *save_ptr;
  char **argv = (char**)malloc(sizeof(char*)*100);
  int total_len = 0;

  strlcpy(file_name_src, file_name, strlen(file_name)+1);
  token = strtok_r(file_name_src, " ", &save_ptr);
  while(token){
	  argv[argc++] = token;
  ...
  *esp -= (word_align + 4);
  strlcpy(*esp, argv[argc], strlen(argv[argc])+1);
  ...
  uint32_t argv_head_address = (uint32_t)(*esp);
  *(uint32_t*)(*esp-4) = argv_head_address;
